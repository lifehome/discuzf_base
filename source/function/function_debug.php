...
function debugmessage($ajax = 0) {
	...
	if($m) {
		$m = '<em>內存:</em> <s>'.$m.'</s> bytes'.($mt ? ', 峰值 <s>'.$mt.'</s> bytes' : '').'<br />';
	}
	global $_G;
...
EOF;
	...
	if(file_exists(DISCUZ_ROOT.'./.svn/entries')) {
		...
		$svn = '.r'.$svn[10].' (最後由 '.$svn[11].' 於 '.dgmdate(strtotime($a[1].' '.$a[2]) + $_G['setting']['timeoffset'] * 3600).' 提交)';
	}
	$max = 10;
	$mc = $mco = '';
	...
	$debug .= '
		...
		'<div id="__debugbar_s">
			...
			'<td valign=top>'.
				'<b style="float:left;width:1em;height:5em">服務器</b>'.
					'<em>環境:</em> '.PHP_OS.', '.$_SERVER['SERVER_SOFTWARE'].' MySQL/'.DB::result_first("SELECT VERSION()").'('.(DB::$driver).')<br />'.
					$m.
					'<em>SQL:</em> '.
			...
			'<tr><td valign=top colspan="2">'.
				'<b>客戶端</b> <a id="__debug_2" href="#debugbar" onclick="switchTab(\'__debug\', 2, '.$max.')">[詳情]</a> <span id="__debug_b"></span>'.
		...
		($_G['adminid'] == 1 ?
			...
			'<img src="../static/image/common/arw_r.gif" /><a href="'.$debugfile.'?k='.$akey.'&'.$mysqlplek.'" target="_blank">MySQL 進程列表</a>'.
			'<img src="../static/image/common/arw_r.gif" /><a href="'.$debugfile.'?k='.$akey.'&'.$viewcachek.'" target="_blank">查看緩存</a>'.
			'<img src="../static/image/common/arw_r.gif" /><a href="../misc.php?mod=initsys&formhash='.formhash().'" target="_debug_initframe" onclick="parent.$(\'_debug_initframe\').onload = function () {parent.location.href=parent.location.href;}">更新緩存</a>' : '').
