...
if($_GET['step'] == 'start') {
	...
	if(version_compare($version, '1.6.0') <= 0) {
		show_msg('請先升級 UCenter 到 1.7.0 及以上版本。<br>如果使用為Discuz! F自帶UCenter，請先運行 <a href="../uc_server/upgrade/upgrade4.php" target="_blank">UC升級程序</a> 進行升級！');
	} else {
	...
	if(empty($_GET['op']) || $_GET['op'] == 'realname') {
		...
		if(save_config_file($configfile, $_config, $default_config, $deletevar)) {
			show_msg("數據處理完成", "$theurl?step=style");
		} else {
			show_msg('"config/config_global.php" 文件已更新，由於 "config/" 目錄不可寫入，我們已將更新的文件保存到 "data/" 目錄下，請通過 FTP 軟件將其轉移到 "config/" 目錄下覆蓋源文件。<br /><br /><a href="'.$theurl.'?step=style">當您完成上述操作後點擊這裡繼續</a>');
		}
	}

} elseif ($_GET['step'] == 'style') {
	if(empty($_GET['confirm'])) {
		show_msg("請確認是否要恢復默認風格？<br /><br /><a href=\"$theurl?step=style&confirm=yes".($_GET['from'] ? '&from='.rawurlencode($_GET['from']).'&frommd5='.rawurlencode($_GET['frommd5']) : '')."\">[ 是 ]</a>&nbsp;&nbsp;<a href=\"$theurl?step=pluginsdisable".($_GET['from'] ? '&from='.rawurlencode($_GET['from']).'&frommd5='.rawurlencode($_GET['frommd5']) : '')."\">[ 否 ]</a>", '');
	}
	...
	show_msg("默認風格已恢復，進入下一步", "$theurl?step=pluginsdisable");

} elseif ($_GET['step'] == 'pluginsdisable') {
	if(empty($_GET['confirm'])) {
		show_msg("關閉所有非系統插件以確保正常運行，請站長逐個開啟每一個插件檢測是否兼容新版本。<br><br>請確認是否要關閉所有非系統插件？<br /><br /><a href=\"$theurl?step=pluginsdisable&confirm=yes".($_GET['from'] ? '&from='.rawurlencode($_GET['from']).'&frommd5='.rawurlencode($_GET['frommd5']) : '')."\">[ 是 ]</a>&nbsp;&nbsp;<a href=\"$theurl?step=delete".($_GET['from'] ? '&from='.rawurlencode($_GET['from']).'&frommd5='.rawurlencode($_GET['frommd5']) : '')."\">[ 否 ]</a>", '');
	}

	DB::query("UPDATE ".DB::table('common_plugin')." SET available='0' WHERE modules NOT LIKE '%s:6:\"system\";i:2;%'");

	show_msg("非系統插件已關閉，進入下一步", "$theurl?step=delete");
	...
	if(empty($deltables) && empty($delcolumns)) {
		echo "<p>與標準數據庫相比，沒有需要刪除的數據表和字段</p><a href=\"$theurl?step=cache".($_GET['from'] ? '&from='.rawurlencode($_GET['from']).'&frommd5='.rawurlencode($_GET['frommd5']) : '')."\">請點擊進入下一步</a></p>";
	} else {
		echo "<p><input type=\"submit\" name=\"delsubmit\" value=\"提交刪除\"></p><p>您也可以忽略多餘的表和字段<br><a href=\"$theurl?step=cache".($_GET['from'] ? '&from='.rawurlencode($_GET['from']).'&frommd5='.rawurlencode($_GET['frommd5']) : '')."\">直接進入下一步</a></p>";
	}
	echo '</form>';
